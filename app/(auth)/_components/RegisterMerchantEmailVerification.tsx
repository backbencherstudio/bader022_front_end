...
import { useCreateAccount } from "../create-account/context/CreateAccount";
...
type EmailVerificationProps = {
  email: string;
  setIsEmailVerification: React.Dispatch<React.SetStateAction<boolean>>;
};

export default function RegisterMerchantEmailVerification({
  email,
  ...
  const [registerVerify, { isLoading }] = useRegisterMerchantVerifyMutation();
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900 px-4">
      <div className="w-full max-w-md bg-white dark:bg-gray-800 rounded-xl shadow-md p-8">
        ...
        <h2 className="text-xl font-semibold text-center text-gray-900 dark:text-white py-4">
          {locale == "ar" ? "أدخل رمز التحقق" : "Enter OTP "}
        </h2>
        <p className="text-sm text-gray-500 dark:text-gray-400 text-center mb-6">
          {locale == "ar"
            ? "لقد أرسلنا رمزًا إلى عنوان بريدك الإلكتروني المسجل"
